...
# db_subnet_group_name       = var.create_subnet_group_name ? aws_db_subnet_group.this.name : var.db_subnet_group_name
  ...
  # monitoring_role_arn                   = aws_iam_role.enhanced_monitoring[0].arn ## Se descomentar esta linha tambem descomente os reacursos associados a ele à partir da linha 63.
  multi_az                              = false
  network_type                          = "IPV4"
